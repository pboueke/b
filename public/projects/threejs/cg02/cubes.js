...
function openControls() {
    swal({   title: "Controls!",   text: "'c' - create a new cube <br>'x' - delete hovered cube <br> 'space bar' - toggle rotation mode <br>",   html: true });
}

function toggleMode() {
    if (ROTATION == false) {
        ROTATION = true;
        document.getElementById("displayMode").value = "Rotation Mode   "; 
        return;
    } else {
        ROTATION = false;
        document.getElementById("displayMode").value = "Translation Mode";
        return;
    }
}
...
function onDocumentMouseDown( event ) {
    ...
    if ( intersects.length > 0 ) {
        ...
        rotateStartPoint = rotateEndPoint = projTrackball(0, 0);
    }
    if ( ROTATION ){
        ROTATING = true;
    }
...
function projTrackball(touchX, touchY) {
...
var rotate = function() {
    rotateEndPoint = projTrackball(deltaX, deltaY);
    var rotatingQuartenion = rotateMatrix(rotateStartPoint, rotateEndPoint);
    curQuaternion = SELECTED.quaternion;
    curQuaternion.multiplyQuaternions(rotatingQuartenion, curQuaternion);
    curQuaternion.normalize();
    SELECTED.setRotationFromQuaternion(curQuaternion);
    rotateEndPoint = rotateStartPoint;
};

function rotateMatrix(start, end) {
